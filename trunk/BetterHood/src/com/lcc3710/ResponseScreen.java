...
public class ResponseScreen extends Activity {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case BetterHood.REQ_OUT_RESPONSE:
			final String response = data.getExtras().getString(
					BetterHood.EXTRAS_WEB_RESPONSE);
			...
			if (resultCode == RESULT_OK) {
				...
				ArrayAdapter<String> adapter2 = new ArrayAdapter<String>(a,
	...
	public void onCreate(Bundle icicle) {
		...
		intent = getIntent();
		extras = intent.getExtras();
		Log.i("gettin dat info=", extras
				.getString(BetterHood.EXTRAS_EVENT_SIMILAR));

		partyTokens = extras.getString(BetterHood.EXTRAS_EVENT_SIMILAR).split(
				delims);
		...
		ArrayAdapter<String> adapter = new ArrayAdapter<String>(a,
				android.R.layout.simple_list_item_multiple_choice, itemsName);
		...
		ts1.setContent(new TabHost.TabContentFactory() {
			public View createTabContent(String tag) {
				...
				inSettings.putExtra(BetterHood.EXTRAS_ACCOUNT_USERNAME, extras
						.getString(BetterHood.EXTRAS_ACCOUNT_USERNAME));
				inSettings.putExtra(BetterHood.EXTRAS_REQUEST_CODE,
						BetterHood.REQ_OUT_RESPONSE);
